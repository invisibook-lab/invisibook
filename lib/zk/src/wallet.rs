...
/// Hard-coded N=2 capacity matching `settle_larger.circom`.
const SETTLE_LARGER_N: usize = 2;

/// Inputs for the larger side of a SettleOrder (`locked > fill`, has change).
/// `inputs` are the sender's locked cashes; `change_amount = sum(inputs) - my_fill`
/// where my_fill is whichever side of the trade this proof's sender is on.
/// `other_fill` is the counterparty's fill; `r_match` values must match what
/// the counterparty's settle proof uses (off-chain coordination).
pub struct SettleLargerWitness {
    pub r_my: [u8; 32],
    pub other_fill: u64,
    pub r_other: [u8; 32],
    pub price: u64,
    pub is_token2_sender: bool,
    pub inputs: Vec<(u64, [u8; 32])>,
    pub change_amount: u64,
    pub change_random: [u8; 32],
    pub counterparty_recv_commitment_hex: String,
}

/// Output of `prove_settle_larger`. Hex strings are 64-char lowercase BE; chain
/// re-derives the public-input vector from these + the bound `price` and
/// `is_token2_sender` flag echoed back in the request.
pub struct SettleLargerProof {
    pub my_match_commitment_hex: String,
    pub other_match_commitment_hex: String,
    pub input_commitments_hex: Vec<String>,
    pub change_commitment_hex: String,
    pub proof_json: Value,
    pub public_json: Value,
...
/// Build the witness for `settle_larger.circom` (N=2), run rapidsnark, and
/// return commitments + snarkjs JSON proof.
///
/// Errors out (without invoking the prover) when:
/// * `inputs` is empty or longer than `SETTLE_LARGER_N`
/// * `change_amount > sum(inputs)` (would underflow my_fill)
/// * `is_token2_sender` ↔ price ↔ other_fill ↔ inferred my_fill don't satisfy
///   `fill_t2 == fill_t1 * price` (the circuit will reject anyway, but failing
///   here gives a clearer error).
pub fn prove_settle_larger(
    w: SettleLargerWitness,
    circuit_handle: &TestCircuitHandle,
    zkey: &Path,
) -> Result<SettleLargerProof> {
    if w.inputs.is_empty() || w.inputs.len() > SETTLE_LARGER_N {
        anyhow::bail!(
            "settle_larger takes 1..={SETTLE_LARGER_N} inputs, got {}",
            w.inputs.len()
        );
    }
    let inputs_sum: u64 = w.inputs.iter().map(|(a, _)| *a).sum();
    if inputs_sum < w.change_amount {
        anyhow::bail!(
            "change_amount ({}) exceeds inputs.sum ({inputs_sum})",
            w.change_amount
        );
    }
    let my_fill = inputs_sum - w.change_amount;
    let (fill_t1, fill_t2) = if w.is_token2_sender {
        (w.other_fill, my_fill)
    } else {
        (my_fill, w.other_fill)
    };
    if fill_t2 != fill_t1.checked_mul(w.price).unwrap_or(u64::MAX) {
        anyhow::bail!(
            "cross-leg ratio violated: fill_t2={fill_t2} != fill_t1={fill_t1} * price={}",
            w.price
        );
    }

    // Off-circuit commitments echoed back to chain.
    let my_match = poseidon_commit(my_fill, &w.r_my);
    let other_match = poseidon_commit(w.other_fill, &w.r_other);
    let change_commitment = poseidon_commit(w.change_amount, &w.change_random);
    let input_commitments: Vec<Fr> = w
        .inputs
        .iter()
    ...
    // Pad unused input slot with (0, 0) so VerifyAmounts opens the same Poseidon
    // constraint on the padded slot.
    let zero_random = [0u8; 32];
    let zero_commitment = poseidon_commit(0, &zero_random);
    let zero_random_fr = Fr::from_be_bytes_mod_order(&zero_random);
    let zero_random_dec = fr_to_decimal_string(&zero_random_fr);
    ...
    let input = json!({
        ...
        "other_match_commitment":       fr_to_decimal_string(&other_match),
        "price":                        w.price.to_string(),
        "is_token2_sender":             if w.is_token2_sender { "1" } else { "0" },
        "input_hashes":                 input_hashes_dec,
        "change_commitment":            fr_to_decimal_string(&change_commitment),
        "counterparty_recv_commitment": counterparty_recv_dec,
        "r_my":                         fr_to_decimal_string(&r_my_fr),
        "other_fill":                   w.other_fill.to_string(),
        "r_other":                      fr_to_decimal_string(&r_other_fr),
        "input_amounts":                input_amounts,
        "input_randomness":             input_randomness,
        "change_amount":                w.change_amount.to_string(),
        "change_random":                fr_to_decimal_string(&change_random_fr),
    });

    let wtns = circuit_handle.gen_witness(&input)?;
    let (proof_json, public_json) = run_rapidsnark(zkey, &wtns)?;

    Ok(SettleLargerProof {
        my_match_commitment_hex: fr_to_hex(&my_match),
        other_match_commitment_hex: fr_to_hex(&other_match),
        input_commitments_hex,
        change_commitment_hex: fr_to_hex(&change_commitment),
        proof_json,
        public_json,
...
const SETTLE_SMALLER_N: usize = 2;

/// Inputs for the smaller side (`locked == fill`, no change). The fill is
/// `inputs.sum` — there's no independent fill field.
pub struct SettleSmallerWitness {
    pub r_match: [u8; 32],
    pub inputs: Vec<(u64, [u8; 32])>,
    pub counterparty_recv_commitment_hex: String,
}

pub struct SettleSmallerProof {
    pub match_commitment_hex: String,
    pub input_commitments_hex: Vec<String>,
    pub proof_json: Value,
    pub public_json: Value,
}

/// Build the witness for `settle_smaller.circom`, run rapidsnark, return
/// commitments + snarkjs JSON proof.
pub fn prove_settle_smaller(
    w: SettleSmallerWitness,
    circuit_handle: &TestCircuitHandle,
    zkey: &Path,
) -> Result<SettleSmallerProof> {
    if w.inputs.is_empty() || w.inputs.len() > SETTLE_SMALLER_N {
        anyhow::bail!(
            "settle_smaller takes 1..={SETTLE_SMALLER_N} inputs, got {}",
            w.inputs.len()
        );
    }

    let inputs_sum: u64 = w.inputs.iter().map(|(a, _)| *a).sum();
    let match_commitment = poseidon_commit(inputs_sum, &w.r_match);
    ...
    let input = json!({
        ...
        "input_hashes":                 input_hashes_dec,
        "counterparty_recv_commitment": counterparty_recv_dec,
        "r_match":                      fr_to_decimal_string(&r_match_fr),
        "input_amounts":                input_amounts,
        "input_randomness":             input_randomness,
    });
    ...
    Ok(SettleSmallerProof {
        match_commitment_hex: fr_to_hex(&match_commitment),
        input_commitments_hex,
        proof_json,
        public_json,
    })
