...
// SendOrderSigningDomain domain-separates the SendOrder v4 signing message
// from every other ed25519 message in the system.
const SendOrderSigningDomain = "invisibook-send-order-v4"
...
// ed25519-signs to authorize a SendOrder v4 request. It covers every field
...
func SendOrderSigningMessage(req *SendOrderRequest) []byte {
	...
	protectionPriceStr := ""
	if req.ProtectionPrice != nil {
		protectionPriceStr = req.ProtectionPrice.String()
	}
	var feeBytes [8]byte
	...
	buf = appendSigningField(buf, strconv.Itoa(int(req.Kind)))
	...
	buf = appendSigningField(buf, protectionPriceStr)
	buf = appendSigningField(buf, req.Pubkey)
	buf = appendSigningField(buf, req.Anchor)
	buf = appendSigningField(buf, req.CollateralNullifiers[0])
	buf = appendSigningField(buf, req.CollateralNullifiers[1])
	buf = appendSigningField(buf, req.FeeNullifiers[0])
	buf = appendSigningField(buf, req.FeeNullifiers[1])
	buf = appendSigningField(buf, req.LockedCommitment)
	buf = appendSigningField(buf, string(feeBytes[:]))
	buf = appendSigningField(buf, req.CollateralChangeCommitment)
	buf = appendSigningField(buf, req.FeeChangeCommitment)
	return buf
}
