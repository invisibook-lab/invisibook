...
// settleFixture mirrors `lib/zk/examples/dump_settle_fixture.rs` output.
...
type settleLargerLeg struct {
	MyMatchCommitmentHex    string          `json:"my_match_commitment_hex"`
	OtherMatchCommitmentHex string          `json:"other_match_commitment_hex"`
	InputCommitmentsHex     []string        `json:"input_commitments_hex"`
	ChangeCommitmentHex     string          `json:"change_commitment_hex"`
	IsToken2Sender          bool            `json:"is_token2_sender"`
	ProofJSON               json.RawMessage `json:"proof_json"`
	PublicJSON              []string        `json:"public_json"`
	VKPath                  string          `json:"vk_path"`
}
type settleSmallerLeg struct {
	MatchCommitmentHex  string          `json:"match_commitment_hex"`
	InputCommitmentsHex []string        `json:"input_commitments_hex"`
	ProofJSON           json.RawMessage `json:"proof_json"`
	PublicJSON          []string        `json:"public_json"`
	VKPath              string          `json:"vk_path"`
}
type sharedRecvCommitments struct {
	AliceRecvCommitmentHex string `json:"alice_recv_commitment_hex"`
	BobRecvCommitmentHex   string `json:"bob_recv_commitment_hex"`
}

func loadSettleFixture(t *testing.T) settleFixture {
	t.Helper()
	const path = "/tmp/settle_fixture.json"
	bytes, err := os.ReadFile(path)
	if err != nil {
		t.Skipf("fixture not found at %s — run `cargo run -p zk --example dump_settle_fixture -- %s`", path, path)
	}
	var f settleFixture
	if err := json.Unmarshal(bytes, &f); err != nil {
		t.Fatalf("decoding fixture: %v", err)
...
func TestVerifyGroth16AcceptsValidSettleLargerProof(t *testing.T) {
	fx := loadSettleFixture(t)
	vk, err := LoadVK("settle_larger", fx.Larger.VKPath)
	if err != nil {
		t.Fatalf("loading VK: %v", err)
	}
	if err := VerifyGroth16(vk, string(fx.Larger.ProofJSON), fx.Larger.PublicJSON); err != nil {
		t.Fatalf("verify on a valid settle_larger proof must succeed, got: %v", err)
	}
}
...
func TestVerifyGroth16RejectsTamperedSettleLargerOtherMatch(t *testing.T) {
	...
	tampered := make([]string, len(fx.Larger.PublicJSON))
	copy(tampered, fx.Larger.PublicJSON)
	// public[1] is other_match_commitment in settle_larger
	tampered[1] = bumpLastDigit(tampered[1])
	if err := VerifyGroth16(vk, string(fx.Larger.ProofJSON), tampered); err == nil {
		t.Fatalf("verify must reject when other_match_commitment is altered")
	}
}

func TestSettleCrossLegMatchEquality(t *testing.T) {
	fx := loadSettleFixture(t)
	if fx.Larger.OtherMatchCommitmentHex != fx.Smaller.MatchCommitmentHex {
		t.Fatalf("fixture is malformed — larger.other_match should equal smaller.match")
	}
}
