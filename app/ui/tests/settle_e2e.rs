...
# Every proof this trade produces is verified on chain: the PLONK
# comparison, both Groth16 settle legs, and each order's send_order proof.
[orderbook]
...
async fn settle_e2e_scenario() {
    ...
    fn session_json(s: &Stats) -> serde_json::Value {
        serde_json::json!({
            ...
            "leg_exchange_ms": s.leg_exchange_ms,
            "total_ms": s.total_ms,
    ...
    // ── Machine-readable copy for the experiment harness. Written only
    //    when INVISIBOOK_E2E_STATS names an output path, so a plain test
    ...
    if let Ok(stats_out) = std::env::var("INVISIBOOK_E2E_STATS") {
        ...
        .expect("write INVISIBOOK_E2E_STATS report");
        eprintln!("stats written to {stats_out}");
    }
