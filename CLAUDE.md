...
- 点对点通信：使用quic协议，用rust实现打洞和连接，给lib下其他需要点对点通信的组件使用
- 多方安全计算：用rust实现edabits，库名使用edabits，专门用来隐私比较两个Uint64的大小，其中beaver AND给我用不经意传输来分配三元组
- 不经意传输：实现 1-out-of-2的不经意传输协议，供多方安全计算调用
...
1. **格式化**：代码写完后运行 `cargo fmt --all`。

2. **导入方式**：struct 和 function 必须通过 `use` 导入后直接使用，禁止通过 `::` 路径内联引用。
...
3. **注释与文档语言**：所有注释（`//`、`///`）和文档字符串一律使用英语。

4. **函数注释**：每个函数添加简短英文注释，说明其作用，以及对入参的隐含要求（如非空、已排序、特定范围等）。

   ```rust
   /// Parses a byte slice into a UTF-8 string.
   /// `bytes` must be valid UTF-8.
   fn parse_str(bytes: &[u8]) -> &str { ... }
   ```

5. **中间过程注释**：复杂逻辑的关键步骤需添加英文行内注释，说明该步骤的意图或非显而易见的原因。

6. rust 代码中struct和函数通过import导入，禁止直接用路径内联引用。

## 工作流程

1. **主动提问**：若给出的 prompt 信息不足以支撑正确完成任务（需求模糊、边界条件不明、依赖上下文缺失等），须主动询问，而不是做出假设后直接推进。

## Skills

### 处理重复模式

当发现跨多处出现的重复逻辑或结构时，将其提炼为独立的函数、trait、或类型，而非就地复制。提炼标准：

- 相同逻辑出现 **2 次以上**；
- 提炼后语义更清晰，或便于独立测试；
- 不以消除重复为目的强行泛化，避免过度抽象。