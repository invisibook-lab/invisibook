...
> **Status:** Current (2026-08-16). Compares the code on this branch with
...
discovery, committed quantities, a maliciously secure 2-party comparison
...
is a public input of the proof (3 public signals: `cmp`, `cm_qA`,
`cm_qB`), both traders ed25519-sign the canonical message
...
The settle circuits open this commitment in a fixed 2-slot shape
(`[LockedCommitment, Poseidon(0,0)]`).
...
`o'_B`. The implementation keeps the same order id and swaps its two
commitments (`Amount` → `cm_q_residual`,
`LockedCommitment` → `cm_locked_residual`) under fresh blindings, then
re-enters matching with the original block height. Privacy is the same —
fresh randomness makes the residual commitments unlinkable to the old
values — and the trader keeps its time priority, which the paper's
...
## 3. Paper mechanisms with no code counterpart
